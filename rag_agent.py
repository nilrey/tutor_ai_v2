...
from langchain_core._api import LangChainDeprecationWarning  # Подавляем лишние warnings
...
def create_rag_chat_chain(retriever, llm):
    ...
    rag_prompt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                """Ты - эксперт по истории. Отвечай на вопрос, используя ТОЛЬКО контекст.
...
{context}""",
    ...
    def get_context(input_data):
        ...
        query = (
            input_data if isinstance(input_data, str) else input_data.get("input", "")
        )
...
def create_simple_chat_chain(llm):
    ...
    simple_prompt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                "Ты - эксперт по истории. Отвечай на вопросы дружелюбно и по делу.",
            ),
            MessagesPlaceholder(variable_name="history"),
            ("human", "{input}"),
        ]
