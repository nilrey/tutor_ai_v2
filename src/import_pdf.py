        ...
        for page_num, page in enumerate(pdf.pages, 1):
            try:
                ...
                if text:
                    ...
                    print(f"  Стр. {page_num}: {chars_before} -> {chars_after} символов (удалено {chars_before - chars_after})")
                else:
                    page_info.append({
                        'page': page_num,
                        'chars_before': 0,
...
def save_intermediate_text(text: str, page_info: List[dict], output_dir: str = "tmp") -> str:
    """
    ...
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write("=" * 80 + "\n")
        f.write(f"ИМПОРТ PDF: Гай_Юлий_Цезарь.pdf\n")
        ...
        for info in page_info:
            if info['chars_before'] > 0:
                f.write(f"Стр. {info['page']:3d}: {info['chars_before']:6d} -> {info['chars_after']:6d} символов (удалено {info['removed']:4d})\n")
        
        f.write("-" * 40 + "\n")
        f.write(f"ИТОГО: {total_chars_before:6d} -> {total_chars_after:6d} символов (удалено {total_chars_before - total_chars_after})\n")
        f.write(f"Удалено %: {(total_chars_before - total_chars_after) / total_chars_before * 100:.1f}%\n\n")
...
def chunk_documents(text: str, chunk_size: int = 800, chunk_overlap: int = 100) -> List[Document]:
    ...
    print(f"Создано {len(documents)} чанков (пропущено {len(chunks) - len(documents)} пустых/коротких)")
    return documents


def save_chunks_info(documents: List[Document], output_dir: str = "tmp", original_text_file: str = None):
...
def add_to_vectorstore(documents: List[Document], persist_directory: str = "./chroma_db"):
